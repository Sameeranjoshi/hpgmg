...
void IterativeSolver(level_type * level, int u_id, int f_id, float a, float b, float desired_reduction_in_norm){ 
  ...
  if(level->must_subtract_mean==-1){
    ...
    int alpha_is_zero = (dot(level,VECTOR_ALPHA,VECTOR_ALPHA) == 0.0f);
  ...
  if( (level->dim.i==1)&&(level->dim.j==1)&&(level->dim.k==1) ){
    ...
    // u = A^{-1}f == D^{-1}f
    mul_vectors(level,u_id,1.0f,VECTOR_DINV,f_id); // u = A^{-1}f = D^{-1}f 
    if(level->must_subtract_mean == 1){
      float mean_of_u = mean(level,u_id);
  ...
  //#else 
    ...
    //mul_vectors(level,VECTOR_TEMP,1.0f,VECTOR_TEMP,VECTOR_DINV); //  Using ||D^{-1}(b-Ax)||_{inf} as convergence criteria...
    float norm_of_r0 = norm(level,VECTOR_TEMP);
    int s=0,maxSmoothsBottom=6,converged=0;
    while( (s<maxSmoothsBottom) ){
      ...
      //residual(level,VECTOR_TEMP,u_id,f_id,a,b);
      //mul_vectors(level,VECTOR_TEMP,1.0f,VECTOR_TEMP,VECTOR_DINV); //  Using ||D^{-1}(b-Ax)||_{inf} as convergence criteria...
      //float norm_of_r = norm(level,VECTOR_TEMP);
      //if(norm_of_r == 0.0f){converged=1;break;}
