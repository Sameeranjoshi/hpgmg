...
void MGPrintTiming(mg_type *all_grids, int fromLevel){
  ...
  REAL SecondsPerCycle = (REAL)1.0/(REAL)(_timeEnd-_timeStart);
  #else
  REAL SecondsPerCycle = 1.0;
  #endif
  REAL scale = SecondsPerCycle/(REAL)all_grids->MGSolves_performed; // prints average performance per MGSolve
  ...
  total=0;printf("smooth                    ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.smooth;               total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("residual                  ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.residual;             total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("applyOp                   ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.apply_op;             total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("BLAS1                     ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.blas1;                total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("BLAS3                     ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.blas3;                total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("Boundary Conditions       ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.boundary_conditions;  total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("Restriction               ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.restriction_total;    total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  local restriction       ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.restriction_local;    total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  #ifdef USE_MPI
  total=0;printf("  pack MPI buffers        ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.restriction_pack;     total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  unpack MPI buffers      ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.restriction_unpack;   total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Isend               ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.restriction_send;     total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Irecv               ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.restriction_recv;     total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Waitall             ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.restriction_wait;     total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  #endif
  total=0;printf("Interpolation             ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.interpolation_total;  total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  local interpolation     ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.interpolation_local;  total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  #ifdef USE_MPI
  total=0;printf("  pack MPI buffers        ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.interpolation_pack;   total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  unpack MPI buffers      ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.interpolation_unpack; total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Isend               ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.interpolation_send;   total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Irecv               ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.interpolation_recv;   total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Waitall             ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.interpolation_wait;   total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  #endif
  total=0;printf("Ghost Zone Exchange       ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.ghostZone_total;      total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  local exchange          ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.ghostZone_local;      total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  #ifdef USE_MPI
  total=0;printf("  pack MPI buffers        ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.ghostZone_pack;       total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  unpack MPI buffers      ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.ghostZone_unpack;     total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Isend               ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.ghostZone_send;       total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Irecv               ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.ghostZone_recv;       total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  total=0;printf("  MPI_Waitall             ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.ghostZone_wait;       total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  #endif
  #ifdef USE_MPI
  total=0;printf("MPI_collectives           ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.collectives;          total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);
  #endif
  total=0;printf("------------------        ");for(level=fromLevel;level<(num_levels+1);level++){printf("------------ ");}printf("\n");
  total=0;printf("Total by level            ");for(level=fromLevel;level<(num_levels  );level++){time=scale*(REAL)all_grids->levels[level]->timers.Total;                total+=time;printf("%12.6f ",time);}printf("%12.6f\n",total);

  printf("\n");
  printf( "   Total time in MGBuild  %12.6f seconds\n",SecondsPerCycle*(REAL)all_grids->timers.MGBuild);
  printf( "   Total time in MGSolve  %12.6f seconds\n",scale*(REAL)all_grids->timers.MGSolve);
...
void build_interpolation(mg_type *all_grids){
  ...
  if( (level>0) && (all_grids->levels[level]->num_my_boxes>0) ){ // not top  *and*  I have boxes to send
    ...
    all_grids->levels[level]->interpolation.send_buffers  =        (REAL**)malloc(numFineRanks*sizeof(REAL*));
    if(numFineRanks>0){
    ...
    //printf("level=%d, rank=%2d, send_buffers=%6d\n",level,all_grids->my_rank,numFineBoxesRemote*elementSize*sizeof(REAL));
    ...
    for(neighbor=0;neighbor<numFineRanks;neighbor++){
      ...
      for(fineBox=0;fineBox<numFineBoxes;fineBox++)if(fineBoxes[fineBox].recvRank==fineRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->interpolation.send_buffers[neighbor] = (REAL*)um_malloc(malloc_size*sizeof(REAL),UM_ACCESS_BOTH);
      ...
      memset(all_grids->levels[level]->interpolation.send_buffers[neighbor],0,malloc_size*sizeof(REAL)); // DO NOT DELETE... you must initialize to 0 to avoid getting something like 0.0*NaN and corrupting the solve
  ...
  if( (level<all_grids->num_levels-1) && (all_grids->levels[level]->num_my_boxes>0) ){ // not bottom  *and*  I have boxes to receive
    ...
    all_grids->levels[level]->interpolation.recv_buffers  =        (REAL**)malloc(numCoarseRanks*sizeof(REAL*));
    if(numCoarseRanks>0){
    ...
    //printf("level=%d, rank=%2d, recv_buffers=%6d\n",level,all_grids->my_rank,numCoarseBoxes*elementSize*sizeof(REAL));
    ...
    for(neighbor=0;neighbor<numCoarseRanks;neighbor++){
      ...
      for(coarseBox=0;coarseBox<numCoarseBoxes;coarseBox++)if(coarseBoxes[coarseBox].sendRank==coarseRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->interpolation.recv_buffers[neighbor] = (REAL*)um_malloc(malloc_size*sizeof(REAL),UM_ACCESS_BOTH); 
      ...
      memset(all_grids->levels[level]->interpolation.recv_buffers[neighbor],0,malloc_size*sizeof(REAL)); // DO NOT DELETE... you must initialize to 0 to avoid getting something like 0.0*NaN and corrupting the solve
...
void build_restriction(mg_type *all_grids, int restrictionType){
  ...
  if( (level<all_grids->num_levels-1) && (all_grids->levels[level]->num_my_boxes>0) ){ // not bottom  *and*  I have boxes to send
    ...
    all_grids->levels[level]->restriction[restrictionType].send_buffers  =        (REAL**)malloc(numCoarseRanks*sizeof(REAL*));
    if(numCoarseRanks>0){
    ...
    for(neighbor=0;neighbor<numCoarseRanks;neighbor++){
      ...
      for(coarseBox=0;coarseBox<numCoarseBoxes;coarseBox++)if(coarseBoxes[coarseBox].recvRank==coarseRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->restriction[restrictionType].send_buffers[neighbor] = (REAL*)um_malloc(malloc_size*sizeof(REAL),UM_ACCESS_BOTH);
      ...
      memset(all_grids->levels[level]->restriction[restrictionType].send_buffers[neighbor],0,malloc_size*sizeof(REAL)); // DO NOT DELETE... you must initialize to 0 to avoid getting something like 0.0*NaN and corrupting the solve
  ...
  if( (level>0) && (all_grids->levels[level]->num_my_boxes>0) ){ // not top  *and*  I have boxes to receive
    ...
    all_grids->levels[level]->restriction[restrictionType].recv_buffers  =        (REAL **)malloc(numFineRanks*sizeof(REAL *));
    if(numFineRanks>0){
    ...
    for(neighbor=0;neighbor<numFineRanks;neighbor++){
      ...
      for(fineBox=0;fineBox<numFineBoxesRemote;fineBox++)if(fineBoxes[fineBox].sendRank==fineRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->restriction[restrictionType].recv_buffers[neighbor] = (REAL *)um_malloc(malloc_size*sizeof(REAL),UM_ACCESS_BOTH);
      ...
      memset(all_grids->levels[level]->restriction[restrictionType].recv_buffers[neighbor],0,malloc_size*sizeof(REAL)); // DO NOT DELETE... you must initialize to 0 to avoid getting something like 0.0*NaN and corrupting the solve
...
//if(fine_grid->dim.k<coarse_dim)coarse_dim = fine_grid->dim.k;
  ...
  if(all_grids->my_rank==0){fprintf(stdout,"\n");}
  for(level=1;level<all_grids->num_levels;level++){
    REAL comm_split_start = MPI_Wtime();
    ...
    REAL comm_split_end = MPI_Wtime();
    REAL comm_split_time_send = comm_split_end-comm_split_start;
    REAL comm_split_time = 0;
    MPI_Allreduce(&comm_split_time_send,&comm_split_time,1,MPI_REAL_TYPE,MPI_MAX,MPI_COMM_WORLD);
  ...
  all_grids->timers.MGBuild += (REAL)(getTime()-_timeStartMGBuild);
}
...
void richardson_error(mg_type *all_grids, int levelh, int u_id){
  ...
  REAL norm_of_u2h_minus_uh  = norm(all_grids->levels[levelh+1],VECTOR_TEMP); // || u^2h - R u^h  ||max
  REAL norm_of_u4h_minus_u2h = norm(all_grids->levels[levelh+2],VECTOR_TEMP); // || u^4h - R u^2h ||max
...
//------------------------------------------------------------------------------------------------------------------------------
void MGVCycle(mg_type *all_grids, int e_id, int R_id, REAL a, REAL b, int level){
...
void MGSolve(mg_type *all_grids, int onLevel, int u_id, int F_id, REAL a, REAL b, REAL dtol, REAL rtol){
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef _OPENMP
  REAL MG_Start_Time = omp_get_wtime();
  #elif USE_MPI
  REAL MG_Start_Time = MPI_Wtime();
  #endif
  ...
  for(v=0;v<maxVCycles;v++){   
    ...
    if(all_grids->levels[level]->must_subtract_mean == 1){
      REAL average_value_of_e = mean(all_grids->levels[level],e_id);
    ...
    REAL norm_of_residual = norm(all_grids->levels[level],VECTOR_TEMP);
    REAL _timeNorm = getTime();
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  all_grids->timers.MGSolve += (REAL)(getTime()-_timeStartMGSolve);
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef _OPENMP
...
void FMGSolve(mg_type *all_grids, int onLevel, int u_id, int F_id, REAL a, REAL b, REAL dtol, REAL rtol){
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef _OPENMP
  REAL FMG_Start_Time = omp_get_wtime();
  #elif USE_MPI
  REAL FMG_Start_Time = MPI_Wtime();
  #endif
...
void MGPCG(mg_type *all_grids, int onLevel, int x_id, int F_id, REAL a, REAL b, REAL dtol, REAL rtol){
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef _OPENMP
  REAL MGPCG_Start_Time = omp_get_wtime();
  #elif USE_MPI
  REAL MGPCG_Start_Time = MPI_Wtime();
  #endif
  ...
  if(level->must_subtract_mean == 1){
    REAL mean_of_r = mean(level,r_id);
    shift_vector(level,r_id,r_id,-mean_of_r);
  }
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  REAL norm_of_r0 = norm(level,r_id);                                         // the norm of the initial residual...
  ...
  while( (j<jMax) && (!CGFailed) && (!CGConverged) ){                           // while(not done){
    ...
    //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  //REAL norm_of_r = norm(level,r_id);                                        //   norm of intermediate residual (delusional convergence)
    residual(level,VECTOR_TEMP,x_id,F_id,a,b);                                  //   true residual
    REAL norm_of_r = norm(level,VECTOR_TEMP);                                 //   norm of true residual (true convergence test)
    ...
    zero_vector(level,z_id);                                                    //   z[] = 0
    MGVCycle(all_grids,z_id,r_id,a,b,onLevel);                                  //   z[] = M^{-1}r[]
    REAL r_dot_z_new = dot(level,r_id,z_id);                                  //   r_dot_z_new = dot(r_{j+1},z_{j+1})
    if(r_dot_z_new == 0.0){CGFailed=1;break;}                                   //   Lanczos breakdown ???
    REAL beta = (r_dot_z_new/r_dot_z);                                        //   beta = (r_dot_z_new/r_dot_z)
