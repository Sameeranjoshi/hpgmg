  ...
  for(s=0;s<CHEBYSHEV_DEGREE*NUM_SMOOTHS;s+=ghosts){
    ...
    for(box=0;box<level->num_my_boxes;box++){
      ...
      for(ss=s;ss<s+ghosts;ss++,ghostsToOperateOn--){
        ...
        const REAL c1 = chebyshev_c1[ss%CHEBYSHEV_DEGREE]; // limit polynomial to degree CHEBYSHEV_DEGREE.
        const REAL c2 = chebyshev_c2[ss%CHEBYSHEV_DEGREE]; // limit polynomial to degree CHEBYSHEV_DEGREE.
