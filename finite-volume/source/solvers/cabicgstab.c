...
// z[r] = alpha*A[r][c]*x[c]+beta*y[r]   // [row][col]
#define gemv(z,alpha,A,x,beta,y,rows,cols)  {int r,c;float sum;for(r=0;r<(rows);r++){sum=0.0f;for(c=0;c<(cols);c++){sum+=(A)[r][c]*(x)[c];}(z)[r]=(alpha)*sum+(beta)*(y)[r];}}
static inline void axpy(float * z, float alpha, float * x, float beta, float * y, int n){ // z[n] = alpha*x[n]+beta*y[n]
  int nn;
  for(nn=0;nn<n;nn++){
    z[nn] = alpha*x[nn] + beta*y[nn];
  }
}
static inline float vdotv(float * x, float * y, int n){ // x[n].y[n]
  int nn;
  float sum = 0.0f;
  for(nn=0;nn<n;nn++){
    sum += x[nn]*y[nn];
...
static inline void zero(float * z, int n){ // z[n] = 0.0
  int nn;
  for(nn=0;nn<n;nn++){
    z[nn] = 0.0f;
  }
...
void CABiCGStab(level_type * level, int e_id, int R_id, float a, float b, float desired_reduction_in_norm){
  ...
  float  temp1[4*CA_KRYLOV_S+1];                                                               //
  float  temp2[4*CA_KRYLOV_S+1];                                                               //
  float  temp3[4*CA_KRYLOV_S+1];                                                               //
  float     Tp[4*CA_KRYLOV_S+1][4*CA_KRYLOV_S+1];                                              // T'  indexed as [row][col]
  float    Tpp[4*CA_KRYLOV_S+1][4*CA_KRYLOV_S+1];                                              // T'' indexed as [row][col]
  float     aj[4*CA_KRYLOV_S+1];                                                               //
  float     cj[4*CA_KRYLOV_S+1];                                                               //
  float     ej[4*CA_KRYLOV_S+1];                                                               //
  float   Tpaj[4*CA_KRYLOV_S+1];                                                               //
  float   Tpcj[4*CA_KRYLOV_S+1];                                                               //
  float  Tppaj[4*CA_KRYLOV_S+1];                                                               //
  float      G[4*CA_KRYLOV_S+1][4*CA_KRYLOV_S+1];                                              // extracted from first 4*CA_KRYLOV_S+1 columns of Gg[][].  indexed as [row][col]
  float      g[4*CA_KRYLOV_S+1];                                                               // extracted from last [4*CA_KRYLOV_S+1] column of Gg[][].
  float    Gg[(4*CA_KRYLOV_S+1)*(4*CA_KRYLOV_S+2)];                                            // buffer to hold the Gram-like matrix produced by matmul().  indexed as [row*(4*CA_KRYLOV_S+2) + col]
  ...
  float g_dot_Tpaj,alpha,omega_numerator,omega_denominator,omega,delta,delta_next,beta;
  float L2_norm_of_rt,L2_norm_of_residual,cj_dot_Gcj,L2_norm_of_s;

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  residual(level,rt_id,e_id,R_id,a,b);                                                           // rt[] = R_id[] - A(e_id)... note, if DPC, then rt = R-AD^-1De
  scale_vector(level,r_id,1.0f,rt_id);                                                               // r[] = rt[]
  scale_vector(level, p_id,1.0f,rt_id);                                                               // p[] = rt[]
  float norm_of_rt = norm(level,rt_id);                                                         // the norm of the initial residual...
  #ifdef VERBOSE
  if(level->my_rank==0)ffprintf(stderr,stderr,"m=%8d, norm   =%0.20f\n",m,norm_of_rt);
  #endif
  if(norm_of_rt == 0.0f){BiCGStabConverged=1;}                                                   // entered BiCGStab with exact solution
  delta = dot(level,r_id,rt_id);                                                                  // delta = dot(r,rt)
  if(delta==0.0f){BiCGStabConverged=1;}                                                          // entered BiCGStab with exact solution (square of L2 norm of r_id)
  L2_norm_of_rt = sqrtf(delta);
  ...
  while( (m<mMax) && (!BiCGStabFailed) && (!BiCGStabConverged) ){                               // while(not done){
    ...
    scale_vector(level,P[0],1.0f, p_id);                                                             // P[0] = A^0p =  p_id
    for(n=1;n<2*ca_krylov_s+1;n++){                                                           // naive way of calculating the monomial basis.
      #ifdef KRYLOV_DIAGONAL_PRECONDITION                                                             //
      mul_vectors(level, VECTOR_TEMP,1.0f, VECTOR_DINV,P[n-1]);                                                //   temp[] = Dinv[]*P[n-1]
    ...
    scale_vector(level,R[0],1.0f,r_id);                                                             // R[0] = A^0r = r_id
    for(n=1;n<2*ca_krylov_s;n++){                                                             // naive way of calculating the monomial basis.
      #ifdef KRYLOV_DIAGONAL_PRECONDITION                                                             //
      mul_vectors(level, VECTOR_TEMP,1.0f, VECTOR_DINV,R[n-1]);                                                //   temp[] = Dinv[]*R[n-1]
    ...
    for(i=0;i<4*ca_krylov_s+1;i++)aj[i]=0.0f;aj[              0]=1.0f;                        // initialized based on (3.26f)
    for(i=0;i<4*ca_krylov_s+1;i++)cj[i]=0.0f;cj[2*ca_krylov_s+1]=1.0f;                        // initialized based on (3.26f)
    for(i=0;i<4*ca_krylov_s+1;i++)ej[i]=0.0f;                                                  // initialized based on (3.26f)
    ...
    for(n=0;n<ca_krylov_s;n++){                                                               // for(n=0;n<ca_krylov_s;n++){
      ...
      gemv( Tpaj,   1.0f, Tp,   aj,   0.0f, Tpaj,4*ca_krylov_s+1,4*ca_krylov_s+1);          //    T'aj
      gemv( Tpcj,   1.0f, Tp,   cj,   0.0f, Tpcj,4*ca_krylov_s+1,4*ca_krylov_s+1);          //    T'cj
      gemv(Tppaj,   1.0f,Tpp,   aj,   0.0f,Tppaj,4*ca_krylov_s+1,4*ca_krylov_s+1);          //   T''aj
                       g_dot_Tpaj = vdotv(g,Tpaj,4*ca_krylov_s+1);                            // (g,T'aj)
      if(g_dot_Tpaj == 0.0f){                                                                    // pivot breakdown ???
        #ifdef VERBOSE                                                                        //
        if(level->my_rank==0){ffprintf(stderr,stderr,"g_dot_Tpaj == 0.0f\n");}                                   //
        #endif                                                                                  //
      ...
      gemv(temp1,-alpha,  G, Tpaj,   0.0f,temp1,4*ca_krylov_s+1,4*ca_krylov_s+1);          //  temp1[] =       - alpha*GT'aj
      gemv(temp1,   1.0f,  G,   cj,   1.0f,temp1,4*ca_krylov_s+1,4*ca_krylov_s+1);          //  temp1[] =   Gcj - alpha*GT'aj
      gemv(temp2,-alpha,  G,Tppaj,   0.0f,temp2,4*ca_krylov_s+1,4*ca_krylov_s+1);          //  temp2[] =       − alpha*GT′′aj
      gemv(temp2,   1.0f,  G, Tpcj,   1.0f,temp2,4*ca_krylov_s+1,4*ca_krylov_s+1);          //  temp2[] = GT′cj − alpha*GT′′aj
      axpy(temp3,   1.0f,     Tpcj,-alpha,Tppaj,4*ca_krylov_s+1);                            //  temp3[] =  T′cj − alpha*T′′aj
      ...
      axpy(temp1,   1.0f,     Tpcj,-alpha,Tppaj,4*ca_krylov_s+1);                            //  temp1[] =  (T'cj - alpha*T''aj)
      gemv(temp2,   1.0f,  G,temp1,   0.0f,temp2,4*ca_krylov_s+1,4*ca_krylov_s+1);          //  temp2[] = G(T'cj - alpha*T''aj)
      axpy(temp3,   1.0f,       cj,-alpha, Tpaj,4*ca_krylov_s+1);                            //  temp3[] =     cj - alpha*T'aj
      ...
      axpy(   ej,1.0f,ej,       alpha,   aj,4*ca_krylov_s+1);                                // ej[] = ej[] + alpha*aj[]    

      // calculate the norm of Saad's vector 's' to check intra s-step convergence...
      axpy(temp1,   1.0f,       cj,-alpha, Tpaj,4*ca_krylov_s+1);                            //  temp1[] =   cj - alpha*T'aj
      gemv(temp2,   1.0f,  G,temp1,   0.0f,temp2,4*ca_krylov_s+1,4*ca_krylov_s+1);          //  temp2[] = G(cj - alpha*T'aj)
      ...
      if(L2_norm_of_s<0)L2_norm_of_s=0;else L2_norm_of_s=sqrtf(L2_norm_of_s);                    // finite precision can lead to the norm^2 being < 0 (Demmel says flush to 0.0f)
      #ifdef VERBOSE                                                                          //
      ...
      if(omega_denominator == 0.0f){                                                             // ??? breakdown
        #ifdef VERBOSE                                                                        //
        if(level->my_rank==0){if(omega_denominator == 0.0f)fprintf(stderr,"omega_denominator == 0.0f\n");}//
        #endif                                                                                  //
        BiCGStabFailed=1;break;                                                                 //
      }                                                                                         //
      ...
      axpy(   ej,1.0f,ej,       omega,   cj,4*ca_krylov_s+1);                                // ej[] = ej[] + alpha*aj[] + omega*cj[]
      axpy(   ej,1.0f,ej,-omega*alpha, Tpaj,4*ca_krylov_s+1);                                // ej[] = ej[] + alpha*aj[] + omega*cj[] - omega*alpha*T'aj[]
      axpy(   cj,1.0f,cj,      -omega, Tpcj,4*ca_krylov_s+1);                                // cj[] = cj[] - omega*T'cj[]
      axpy(   cj,1.0f,cj,      -alpha, Tpaj,4*ca_krylov_s+1);                                // cj[] = cj[] - omega*T'cj[] - alpha*T'aj[]
      axpy(   cj,1.0f,cj, omega*alpha,Tppaj,4*ca_krylov_s+1);                                // cj[] = cj[] - omega*T'cj[] - alpha*T'aj[] + omega*alpha*T''aj[]


      // calculate the norm of the incremental residual (Saad's vector 'r') to check intra s-step convergence...
      gemv(temp1,   1.0f,  G,   cj,   0.0f,temp1,4*ca_krylov_s+1,4*ca_krylov_s+1);          // temp1[] = Gcj
                                       cj_dot_Gcj = vdotv(cj,temp1,4*ca_krylov_s+1);          // sqrtf( (cj,Gcj) ) == L2 norm of the intermediate residual in exact arithmetic
      L2_norm_of_residual = 0.0f;if(cj_dot_Gcj>0)L2_norm_of_residual=sqrtf(cj_dot_Gcj);           // finite precision can lead to the norm^2 being < 0 (Demmel says flush to 0.0f)
      #ifdef VERBOSE 
      ...
      if(level->my_rank==0){                                                                    //
        ...
        if(delta_next      == 0.0f){fprintf(stderr,"delta == 0.0f\n");}                                   // Lanczos breakdown
        if(omega_numerator == 0.0f){fprintf(stderr,"omega_numerator == 0.0f\n");}                         // stabilization breakdown
        if(omega           == 0.0f){fprintf(stderr,"omega == 0.0f\n");}                                   // stabilization breakdown 
      }                                                                                         //
      #endif                                                                                    //
      if(isinf(delta_next)){BiCGStabFailed   =1;break;}                                         // delta = inf?
      if(delta_next  ==0.0f){BiCGStabFailed   =1;break;}                                         // Lanczos breakdown...
      if(omega       ==0.0f){BiCGStabFailed   =1;break;}                                         // stabilization breakdown 
      beta = (delta_next/delta)*(alpha/omega);                                                  // (delta_next/delta)*(alpha/omega)
      #ifdef VERBOSE                                                                          //
      if(level->my_rank==0){                                                                    //
        if(isinf(beta)           ){fprintf(stderr,"beta == inf\n");}                                    // beta = inf?
        if(beta            == 0.0f){fprintf(stderr,"beta == 0.0f\n");}                                    // beta = 0?  can't make further progress(?)
      }                                                                                         //
      #endif                                                                                    //
      if(isinf(beta)      ){BiCGStabFailed   =1;break;}                                         // beta = inf?
      if(beta       == 0.0f){BiCGStabFailed   =1;break;}                                         // beta = 0?  can't make further progress(?)
      axpy(   aj,1.0f,cj,        beta,   aj,4*ca_krylov_s+1);                                // aj[] = cj[] + beta*aj[]
      axpy(   aj,1.0f,aj, -omega*beta, Tpaj,4*ca_krylov_s+1);                                // aj[] = cj[] + beta*aj[] - omega*beta*T'aj
      delta = delta_next;                                                                       // delta = delta_next

    }                                                                                           // inner n (j) loop

    // update iterates...
    for(i=0;i<4*ca_krylov_s+1;i++){add_vectors(level,e_id,1.0f,e_id,ej[i],PRrt[i]);}             // e_id[] = [P,R]ej + e_id[]
    if(!BiCGStabFailed && !BiCGStabConverged){                                                  // if we're done, then there is no point in updating these
                                   add_vectors(level,  p_id,0.0f,  p_id,aj[0],PRrt[0]);              //    p[] = [P,R]aj
    for(i=1;i<4*ca_krylov_s+1;i++){add_vectors(level,  p_id,1.0f,  p_id,aj[i],PRrt[i]);}             //          ...
                                   add_vectors(level, r_id,0.0f, r_id,cj[0],PRrt[0]);              //    r[] = [P,R]cj
    for(i=1;i<4*ca_krylov_s+1;i++){add_vectors(level, r_id,1.0f, r_id,cj[i],PRrt[i]);}             //          ...
    }                                                                                           //
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  #ifdef KRYLOV_DIAGONAL_PRECONDITION
  mul_vectors(level,e_id,1.0f, VECTOR_DINV,e_id);                                                        //   e_id[] = Dinv[]*e_id[] // i.e. e = D^{-1}e'
  #endif
...
void CABiCGStab(level_type * level, int e_id, int R_id, float a, float b, float desired_reduction_in_norm){
  ...
  while( (m<mMax) && (!BiCGStabFailed) && (!BiCGStabConverged) ){               // while(not done){
    ...
    for(n=0;n<ca_krylov_s;n++){                                                               // for(n=0;n<ca_krylov_s;n++){
      ...
      if(g_dot_Tpaj == 0.0f){                                                                    // pivot breakdown ???
        #ifdef VERBOSE                                                                        //
        if(level->my_rank==0){fprintf(stderr,"g_dot_Tpaj == 0.0f\n");}                                   //
        #endif                                                                                  //
        BiCGStabFailed=1;break;                                                                 //
      }                                                                                         //
      alpha = delta / g_dot_Tpaj;                                                               // delta / (g,T'aj)
