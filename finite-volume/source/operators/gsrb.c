...
//------------------------------------------------------------------------------------------------------------------------------
#if   defined(GSRB_FP)
  #warning Overriding default GSRB implementation and using pre-computed 1.0f/0.0f FP array for Red-Black to facilitate vectorization...
...
void smooth(level_type * level, int x_id, int rhs_id, float a, float b){
  ...
  for(s=0;s<2*NUM_SMOOTHS;s++){ // there are two sweeps per GSRB smooth
    ...
    for(block=0;block<level->num_my_blocks;block++){
      ...
      #else
      const float * __restrict__ x_n      = level->my_boxes[box].vectors[         x_id] + ghosts*(1+jStride+kStride); // i.e. [0] = first non ghost zone point
            float * __restrict__ x_np1    = level->my_boxes[box].vectors[         x_id] + ghosts*(1+jStride+kStride); // i.e. [0] = first non ghost zone point
      #endif
          

      #if defined(GSRB_FP)
      for(k=klo;k<khi;k++){const float * __restrict__ RedBlack = level->RedBlack_FP + ghosts*(1+jStride) + kStride*((k^color000)&0x1);
      for(j=jlo;j<jhi;j++){
      for(i=ilo;i<ihi;i++){
            int ij  = i + j*jStride;
            int ijk = i + j*jStride + k*kStride;
            float Ax     = apply_op_ijk(x_n);
