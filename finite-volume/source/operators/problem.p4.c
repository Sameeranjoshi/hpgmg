...
void evaluateU(float x, float y, float z, float *U, float *Ux, float *Uy, float *Uz, float *Uxx, float *Uyy, float *Uzz, int isPeriodic){
  ...
  float shift = 0.0f;if(isPeriodic)shift= -1.0f/30.0f;
  float X   = 1.0f*powf(x,4) -  2.0f*powf(x,3) + 1.0f*powf(x,2) + shift;
  float Y   = 1.0f*powf(y,4) -  2.0f*powf(y,3) + 1.0f*powf(y,2) + shift;
  float Z   = 1.0f*powf(z,4) -  2.0f*powf(z,3) + 1.0f*powf(z,2) + shift;
  float Xx  =  4.0f*powf(x,3) -  6.0f*powf(x,2) + 2.0f*x;
  float Yy  =  4.0f*powf(y,3) -  6.0f*powf(y,2) + 2.0f*y;
  float Zz  =  4.0f*powf(z,3) -  6.0f*powf(z,2) + 2.0f*z;
  float Xxx = 12.0f*powf(x,2) - 12.0f*x        + 2.0f;
  float Yyy = 12.0f*powf(y,2) - 12.0f*y        + 2.0f;
  float Zzz = 12.0f*powf(z,2) - 12.0f*z        + 2.0f;
        *U   = X*Y*Z;
        *Ux  = Xx*Y*Z;
        *Uy  = X*Yy*Z;
        *Uz  = X*Y*Zz;
