...
void evaluateU(REAL x, REAL y, REAL z, REAL *U, REAL *Ux, REAL *Uy, REAL *Uz, REAL *Uxx, REAL *Uyy, REAL *Uzz, int isPeriodic){
  ...
  REAL shift = 0.0;if(isPeriodic)shift= -1.0/30.0;
  REAL X   =  1.0*pow(x,4) -  2.0*pow(x,3) + 1.0*pow(x,2) + shift;
  REAL Y   =  1.0*pow(y,4) -  2.0*pow(y,3) + 1.0*pow(y,2) + shift;
  REAL Z   =  1.0*pow(z,4) -  2.0*pow(z,3) + 1.0*pow(z,2) + shift;
  REAL Xx  =  4.0*pow(x,3) -  6.0*pow(x,2) + 2.0*x;
  REAL Yy  =  4.0*pow(y,3) -  6.0*pow(y,2) + 2.0*y;
  REAL Zz  =  4.0*pow(z,3) -  6.0*pow(z,2) + 2.0*z;
  REAL Xxx = 12.0*pow(x,2) - 12.0*x        + 2.0;
  REAL Yyy = 12.0*pow(y,2) - 12.0*y        + 2.0;
  REAL Zzz = 12.0*pow(z,2) - 12.0*z        + 2.0;
        *U   = X*Y*Z;
        *Ux  = Xx*Y*Z;
        *Uy  = X*Yy*Z;
        *Uz  = X*Y*Zz;
