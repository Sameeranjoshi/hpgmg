...
//------------------------------------------------------------------------------------------------------------------------------
static inline void interpolation_p0_block(level_type *level_f, int id_f, float prescale_f, level_type *level_c, int id_c, blockCopy_type *block){
  ...
  for(i=0;i<dim_i;i++){
    ...
    write[write_ijk] = prescale_f*write[write_ijk] + read[read_ijk]; // CAREFUL !!!  you must guarantee you zero'd the MPI buffers(write[]) and destination boxes at some point to avoid 0.0f*NaN or 0.0f*inf
  }}}
...
void interpolation_p0(level_type * level_f, int id_f, float prescale_f, level_type *level_c, int id_c){
  ...
  if(level_c->interpolation.num_blocks[0]>0){
    _timeStart = getTime();
    if(level_f->use_cuda) {
      cuda_interpolation_p0(*level_f,id_f,0.0f,*level_c,id_c,level_c->interpolation,0);
    ...
    for(buffer=0;buffer<level_c->interpolation.num_blocks[0];buffer++){
      ...
      interpolation_p0_block(level_f,id_f,0.0f,level_c,id_c,&level_c->interpolation.blocks[0][buffer]);
    }
    }
    _timeEnd = getTime();
  ...
  if(level_c->interpolation.num_blocks[1]>0){
    _timeStart = getTime();
    if (level_f->use_cuda) {
      cuda_interpolation_p0(*level_f, id_f, prescale_f, *level_c, id_c, level_c->interpolation, 1);
    }
    else {
