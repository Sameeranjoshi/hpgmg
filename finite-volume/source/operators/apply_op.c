...
void apply_op(level_type * level, int Ax_id, int x_id, float a, float b){
  ...
  for(block=0;block<level->num_my_blocks;block++){
    ...
    const float * __restrict__ x      = level->my_boxes[box].vectors[         x_id] + ghosts*(1+jStride+kStride); // i.e. [0] = first non ghost zone point
          float * __restrict__ Ax     = level->my_boxes[box].vectors[        Ax_id] + ghosts*(1+jStride+kStride); 
    ...
    const float * __restrict__  valid = level->my_boxes[box].vectors[VECTOR_VALID ] + ghosts*(1+jStride+kStride);
