...
#include <math.h>
//------------------------------------------------------------------------------------------------------------------------------
static inline void interpolation_v2_block(level_type *level_f, int id_f, float prescale_f, level_type *level_c, int id_c, blockCopy_type *block){
  ...
  int i,j,k;
  float c1 = 1.0f/8.0f;
  for(k=0;k<write_dim_k;k++){float c1k=c1;if(k&0x1){c1k=-c1;}
  for(j=0;j<write_dim_j;j++){float c1j=c1;if(j&0x1){c1j=-c1;}
  for(i=0;i<write_dim_i;i++){float c1i=c1;if(i&0x1){c1i=-c1;}
    int write_ijk = ((i   )+write_i) + (((j   )+write_j)*write_jStride) + (((k   )+write_k)*write_kStride);
    int  read_ijk = ((i>>1)+ read_i) + (((j>>1)+ read_j)* read_jStride) + (((k>>1)+ read_k)* read_kStride);
    //
    // |  1/8  |  1.0f  | -1/8  | coarse grid
    // |---+---|---+---|---+---|
    // |   |   |???|   |   |   | fine grid
    //
  ...
  for(i=0;i<write_dim_i;i+=2){
    ...
    const float c000=read[read_ijk-1-read_jStride-read_kStride], c100=read[read_ijk  -read_jStride-read_kStride], c200=read[read_ijk+1-read_jStride-read_kStride];
    const float c010=read[read_ijk-1             -read_kStride], c110=read[read_ijk               -read_kStride], c210=read[read_ijk+1             -read_kStride];
    const float c020=read[read_ijk-1+read_jStride-read_kStride], c120=read[read_ijk  +read_jStride-read_kStride], c220=read[read_ijk+1+read_jStride-read_kStride];
    const float c001=read[read_ijk-1-read_jStride             ], c101=read[read_ijk  -read_jStride             ], c201=read[read_ijk+1-read_jStride             ];
    const float c011=read[read_ijk-1                          ], c111=read[read_ijk                            ], c211=read[read_ijk+1                          ];
    const float c021=read[read_ijk-1+read_jStride             ], c121=read[read_ijk  +read_jStride             ], c221=read[read_ijk+1+read_jStride             ];
    const float c002=read[read_ijk-1-read_jStride+read_kStride], c102=read[read_ijk  -read_jStride+read_kStride], c202=read[read_ijk+1-read_jStride+read_kStride];
    const float c012=read[read_ijk-1             +read_kStride], c112=read[read_ijk               +read_kStride], c212=read[read_ijk+1             +read_kStride];
    const float c022=read[read_ijk-1+read_jStride+read_kStride], c122=read[read_ijk  +read_jStride+read_kStride], c222=read[read_ijk+1+read_jStride+read_kStride];
    ...
    // +-------+-------+-------+      :.......+---+---+.......:
    //
    const float f0c00 = ( c100 + c1*(c000-c200) ); // same as original 3pt stencil... f0c00 = ( c1*c000 + c100 - c1*c200 );
    const float f1c00 = ( c100 - c1*(c000-c200) );
    const float f0c10 = ( c110 + c1*(c010-c210) );
    const float f1c10 = ( c110 - c1*(c010-c210) );
    const float f0c20 = ( c120 + c1*(c020-c220) );
    const float f1c20 = ( c120 - c1*(c020-c220) );

    const float f0c01 = ( c101 + c1*(c001-c201) );
    const float f1c01 = ( c101 - c1*(c001-c201) );
    const float f0c11 = ( c111 + c1*(c011-c211) );
    const float f1c11 = ( c111 - c1*(c011-c211) );
    const float f0c21 = ( c121 + c1*(c021-c221) );
    const float f1c21 = ( c121 - c1*(c021-c221) );

    const float f0c02 = ( c102 + c1*(c002-c202) );
    const float f1c02 = ( c102 - c1*(c002-c202) );
    const float f0c12 = ( c112 + c1*(c012-c212) );
    const float f1c12 = ( c112 - c1*(c012-c212) );
    const float f0c22 = ( c122 + c1*(c022-c222) );
    const float f1c22 = ( c122 - c1*(c022-c222) );
    ...
    // :.......+---+---+.......:      :.......:.......:.......:
    //
    const float f00c0 = ( f0c10 + c1*(f0c00-f0c20) );
    const float f10c0 = ( f1c10 + c1*(f1c00-f1c20) );
    const float f01c0 = ( f0c10 - c1*(f0c00-f0c20) );
    const float f11c0 = ( f1c10 - c1*(f1c00-f1c20) );

    const float f00c1 = ( f0c11 + c1*(f0c01-f0c21) );
    const float f10c1 = ( f1c11 + c1*(f1c01-f1c21) );
    const float f01c1 = ( f0c11 - c1*(f0c01-f0c21) );
    const float f11c1 = ( f1c11 - c1*(f1c01-f1c21) );

    const float f00c2 = ( f0c12 + c1*(f0c02-f0c22) );
    const float f10c2 = ( f1c12 + c1*(f1c02-f1c22) );
    const float f01c2 = ( f0c12 - c1*(f0c02-f0c22) );
    const float f11c2 = ( f1c12 - c1*(f1c02-f1c22) );

    // interpolate in k to create fine ijk points...
    const float f000 = ( f00c1 + c1*(f00c0-f00c2) );
    const float f100 = ( f10c1 + c1*(f10c0-f10c2) );
    const float f010 = ( f01c1 + c1*(f01c0-f01c2) );
    const float f110 = ( f11c1 + c1*(f11c0-f11c2) );
    const float f001 = ( f00c1 - c1*(f00c0-f00c2) );
    const float f101 = ( f10c1 - c1*(f10c0-f10c2) );
    const float f011 = ( f01c1 - c1*(f01c0-f01c2) );
    const float f111 = ( f11c1 - c1*(f11c0-f11c2) );
...
void interpolation_v2(level_type * level_f, int id_f, float prescale_f, level_type *level_c, int id_c){
  ...
  if(level_c->interpolation.num_blocks[0]>0){
    _timeStart = getTime();
    if(level_f->use_cuda) {
      cuda_interpolation_v2(*level_f,id_f,0.0f,*level_c,id_c,level_c->interpolation,0);
    ...
    for(buffer=0;buffer<level_c->interpolation.num_blocks[0];buffer++){
      ...
      interpolation_v2_block(level_f,id_f,0.0f,level_c,id_c,&level_c->interpolation.blocks[0][buffer]);
    }
    }
    _timeEnd = getTime();
